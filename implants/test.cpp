...
#define COMMAND_URL L"http://192.168.16.132/commands/test.json"
#define UPLOAD_URL L"http://192.168.16.132/uploads/test_response.txt"
#define NOTIFY_URL L"http://192.168.16.132/notify.txt"

std::string get_local_command_file()
{
    return fs::temp_directory_path().string() + "test.json";
}

std::string get_local_result_file()
{
    return fs::temp_directory_path().string() + "test_response.txt";
}

std::wstring get_local_command_file_wstring()
{
    return std::wstring(fs::temp_directory_path().wstring() + L"test.json");
}

std::wstring get_local_result_file_wstring()
{
    return std::wstring(fs::temp_directory_path().wstring() + L"test_response.txt");
}
...
int main() {
    ...
    std::ofstream notifyFile("test_notify.txt");
    if (notifyFile.is_open()) {
        notifyFile << "identifier=test";
        notifyFile.close();
        UploadFile(pManager, NOTIFY_URL, L"test_notify.txt");
        std::remove("test_notify.txt");
    }
    while (true) {
